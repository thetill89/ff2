...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let buyMax, buyGoal, minFries, useMaxBuy, otherPct, buyTimeOut, buyStep, mainBuild = 8
let buyTimer = buyStepTimer = idleTimer = updateTimer = sessionTimer = sessionTimer2 = Date.now()
...
function buyRoutine() { 
	...
	if (buyTimeOut > now - buyTimer || getSessionTime() > idleMax + 10 || getIdleTime() < idleTime) return
...
function startAutoPlay(adjust=true) {
	if (!autoPlay) {
		if (adjust) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let resetStep = runData = ep = sp = 0
function resetRoutine() {
	if (getSessionTime() < sessionTime + 1 || idlemode || resetStep) return
	resetStep = 1
	$j.get('game/megaanz.php').done(function(r) {
		runData = [getLevel(), parseFloat(document.getElementsByClassName('epbari')[0].style.width),
		incomeoffline.slice(2,12), r.nextreset, r.maxrelikte]		
...
function adjustProd() {
	let factor, oldProd = maxProd
	if ((Date.now()- autoPlay)/1000 > sessionTime) {
		if (prodExeeded) {
			if (runData[0] < maxLevel) factor = 1.01
			else if (runData[0] > maxLevel || (runData[0] === maxLevel && runData[1] > 80)) factor = 0.99
			if (factor) maxProd = convertValue(getValue(maxProd) * factor)
		}		
		else {
			if (runData[0] > maxLevel || (runData[0] === maxLevel && runData[1] > 80)) factor = 0.99
			if (factor) maxProd = convertValue(getValue(incomeoffline) * factor)
		}	
		if (factor) {
			UI[2].setValue('maxProd', maxProd)
			addStatus2(seperator)
			addStatus2('Before:   ' + oldProd)
			addStatus2('New prod: ' + maxProd)
			addStatus2('Idle end: ' + idleStats[0])
			addStatus2('Lvl '+ runData[0] + ': ' + runData[1].toFixed() + ' %')
			saveSettings()
		}
	}
	if (runData[0] > highLevel) { 
		highLevel = runData[0]
		saveSettings()
	}
}
...
function solveCaptcha(captcha) {
	...
	$j.get('game/resetmd.php?rscode=' + captcha).done(function(r) { 
		finishRun() 
	}).fail(function(r) {
			setSessionTime(10)	
			resetStep = 0
...
let finishAudio = new Audio('http://soundbible.com/mp3/Elevator%20Ding-SoundBible.com-685385892.mp3')
function finishRun() {
	ep = getPoints('event') - ep 
	sp = getPoints('skill') - sp 
	if (sp < 0 || sp > 10 || ep < 30 || ep > 80) sp = ep = 0
	setTodayRuns()
	updateSummary()	
	addLog(padLeft(todayRuns.r, 3) + ' | ' + padLeft(runData[0],5) + ' | ' + padLeft(runData[2],9) + ' | '
	+ padLeft(runData[3],9) + ' | '  + padLeft(runData[4],10) + ' | '
	+ padLeft(sp,2) + ' sp | ' + padLeft(ep,3) + ' ep')
	sessionTimer = sessionTimer2 = Date.now()
	idleTimer = Date.now()
	idleTime = -1
	idleStats = [0,0,[]] 
...
function displayStats() {
	...
	if (isVisible('idlemode')) {
		 let i1 = [], i2 = []
		 let status = autoPlay ? ' CURRENT RUN ' + buyStep + '/' + (buyMax.length+1) : ' CURRENT RUN'
		 i1.push(status + '\n')
		 let steps = autoPlay && idleStats[2].length? [...idleStats[2]].reverse().join('\n') : 
			' Online:  ' + incomepersek.slice(2,12) + '\n Offline: ' + incomeoffline.slice(2,12)
		 i1.push(steps)
		 $j('#idleinfo1').val(i1.join('\n')) 
		  //--------------------------------------
		 if (idlemode) status =  ' IDLE MODE'
		 else status = ' BUYING'
		 i2.push(status +'\n')
		 ...
		 let s = 10
		 //--------TIME
		 if (idlemode) status = ' Time  |  ' + padRight(displayTime(getIdleTime()),s) 
			 + ' |  ' + padRight(time,s-1) 
			 + ' |  ' + (idleStats[0] || '00:00')
		 else status = '  Fries  |  Pizza  |  Hotd   |  Hamb'
		 i2.push(status)
		 //--------COST		 
		 let nextBuilding = autoPlay && buyStep <= buyMax.length ? buyGoal[buyStep-1] : building[mainBuild]+100
		 if (idlemode) status = ' Cost  |  '  + padRight(convertValue(getGold() + getIdleIncome()),s) 
			+ ' |  ' + padRight(convertValue(nextCost),s-1) 
			+ ' |  ' + nextBuilding
		  else  status = '  ' + padRight(total[0][0],5) + '  |  ' + padRight(total[0][1],5) + '  |  ' + padRight(total[0][2],5) + '  |  ' + padRight(total[0][3],5)
		  i2.push(status)
		  //--------PROD
		 if (!idlemode) i2.push('  ' + ('-').repeat(36))
		 let factor = building[mainBuild] ? total[1]/building[mainBuild] : 1
		 if (idlemode) status = ' Prod  |  ' + padRight(incomeoffline.slice(2,12),s) 
			 + ' |  ' + padRight((factor).toFixed(2),s-1)
			 + ' |  ' + total[1]
		 else  status = '  Bike   |  Scoot  |  Van    |  Truck'
		 i2.push(status)
		 if (!idlemode) i2.push('  ' + padRight(total[0][4],5) + '  |  ' + padRight(total[0][5],5) + '  |  ' + padRight(total[0][6],5) + '  |  ' + padRight(total[0][7],5)) 
		 $j('#idleinfo2').val(i2.join('\n'))	
		 if (autoPlay && !idlemode)	idleinfo2.style.outline = '2px solid ' + theme
		 else idleinfo2.style.outline = 'none'
	}
	...
	mTime = mTime >= 3600 ? '∞' : mTime >= 60 ? padLeft(Math.round(mTime/60),2) +  ' min': displayTime(mTime)	
	fTime = farmLeft/1000 - (Date.now() - farmTimer)/1000
	fTime = fTime >= 9600 ? '∞' : fTime >= 60 ? padLeft(Math.round(fTime/60),2) +  ' min': displayTime(fTime)	
	timeinfo.innerHTML = '\nMission: ' + mTime + '\nFarming: ' + fTime
...
function statusInfo(lvl=false) {
	info2.value = ''
	if (lvl) addStatus2('Last:      ' + runData[0] + ', ' + Math.floor(runData[1]) + '%')
	if (todayRuns.c) {
		let avgEP = (todayRuns.ep/todayRuns.c).toFixed(2)
		let avgSP = (todayRuns.sp/todayRuns.c).toFixed(2)
		addStatus2('Ø SP:      ' + avgSP + ', ' + avgEP)
		
	}
	addStatus2(seperator)
	let data = getFarmData()
	let time = (Date.now()-ST.startTime)/1000
	addStatus2('Points:   ' + data[2] + ' | ' + data[3] + ' | ' + data[4])
	addStatus2('Farm:      ' + data[1])
	let runMS = convertValue(getValue(getLogData()[1]) * todayRuns.r)
	addStatus2('Dollar:    ' +  runMS)
	addStatus2(seperator)
	addStatus2('Level:     ' + highLevel)
	addStatus2('Resets:    ' + ST.totalRuns.toLocaleString())
	addStatus2('Playtime:  ' + displayTime(time, true))
	addStatus2('Account:   ' + account)
}
...
function getMissionTime() {
	$j.get('game/clanquests.php').done(function(r) {
		missionLeft = r.clanquestdauer + 1
		missionTimer = Date.now()
		getMissionStanding(r)
		getClanChat()
		if (!autoPlay && autoMission) setTimeout(getMissionTime, 1200000)
	}).fail(() => setTimeout(getMissionTime, failTimeOut * 1000))	
}

function doMission(forced=false) {
	if ((resetStep || !autoMission) && !forced) return
...
function doTour() {
	...
	if (Date.now() - tourTimer > tourLeft * 1000) {
		tourLeft = 1e9
		$j.get('game/clantourfertig.php').done(function(r) {
...
function collectFarm() {
	...
	if (Date.now() - farmTimer > farmLeft) {
		...
		$j.get('game/farmcollect.php').done(function(r) {
			if (r.erfolg === 1) {
				let fp = ' | ' + padLeft(r.megapointswin, 2) 
				let fsp = ' | ' + padLeft(r.skillpointsplus, 2)
				let fep = ' | ' + padLeft(r.eventpointsplus, 2)			
...
function collectNow() {
	...
	if (val === 'Farm') {
		...
		setTimeout(function(){ btn20.innerText = 'Farm' }, 5000)
	}
	else {
		farmLeft = 0
		collectFarm()
		btn20.innerText = 'Farm'
...
function doResearch() {
	...
	if (Date.now() - researchTimer > researchLeft) {
		researchLeft = 1e9
		$j.get('game/researchfertig.php').done(function(r) {
...
function upgradeSkill() {
	if (autoSkill) {
		...
		if (spTmp >= 10) {
			spTmp -= 10
			$j.get('game/skillupgrade.php?10').done(function(r) { 
				$j('.skillbtn').html('Skills: ' + spTmp)
		 })	
		}	
	}
}
...
function getMissionStanding(r) {
	let info = [] 
	let pct = r.aktclanquest ? padLeft(Math.round(getValue(r.aktclanquest)/getValue(r.aktfullpoints)*100) + '%',4) : padLeft('0%',4)
	info.push('\ MISSION' + pct + '\n')
	let max = r.anz < 9 ? r.anz : 9
	for (let i = 1; i <= max; i++) {
		let points = r.aktclandmg[i]
		if (!points) continue
		let level = ' ' + padLeft(r.maxlevelarr[i], 4) + ' | '
		let name = padRight(r.namearray[i].split('> ')[1].slice(0,16), 16) + ' | '
		points = padLeft(points.toLocaleString(), 15)
		info.push(level + name + points)
	}	
	$j('#idleinfo4').val(info.join('\n')) 
}
...
function getClanChat() {
	...
	$j.get('game/sortonline.php').done(function(r) {	
		let info = []
		info.push(' ONLINE\n')
		let max = r.anz < maxEntry-3 ? r.anz : maxEntry-3
		for (let i = 1; i <= max; i++) {
			let nbr =  padLeft(i,2) + ' | '
			let level = padLeft(r.maxlevelarr[i], 5) + '  |'
			let name = padRight(r.namearray[i].match(/\/>\s(\w+)/)[1].slice(0,16), 16) + ' | '
	...
	$j.get('game/clanchat.php').done(function(r) {	
		...
		for (let i = 1; i <= max; i++) {
			...
			let name = padRight(r.gchatfrom[i].slice(0,7), 7) + ' |  '
	...
	$j.get('game/turnieranz.php').done(function(r) {		
		...
		for (let i = 1; i <= max; i++) {
			let rank = padLeft(r.turnierrank[i],2) + ' | '
			let name = padRight(r.playername[i].slice(0,14), 14) + ' | '
			let level = padLeft(r.playermaxlvl[i], 5) + ' | '
			let prize = padLeft(r.turnierprizes[i].slice(0,12), 14)
			info.push(rank + name + level + prize)
		}	
		$j('#idleinfo6').val(info.join('\n')) 
	})	
}
	...
	if(autoReward && (!new Date().getHours() || forced)) {
		$j.get('game/eventanz.php').done(function(r) { 
			if (!r.noreward) {
				...
				$j.get('game/claimreward.php').done(function(r) { 
					homeanz() 
				})	
			}
		})	
...
function setTodayRuns() {
	...
	todayRuns.r++
	localStorage.setItem(todayLog, JSON.stringify(todayRuns))
	ST.totalRuns++
	saveSettings(true)
}
...
function updateSummary() {
	...
	let sum = JSON.parse(decompress(localStorage.getItem(account + '_summary')))
	let p = getValue(runData[2])
	let l = runData[0]
	let m = getValue(runData[3]), t = getValue(runData[4])
	let s = e = c = 0
	if (ep) {
		s = sp
		e = ep
	...
	allRuns = sum
	localStorage.setItem(account + '_summary', compress(JSON.stringify(sum)))
}

let seperator = ('-').repeat(31), maxRuns = 140, allRuns
function statsSummary() {
	if (!allRuns) allRuns = JSON.parse(decompress(localStorage.getItem(account + '_summary')))
	...
	for (key in allRuns) {
		...
		runs = padLeft(allRuns[key].r,4)
		lvl = padLeft(allRuns[key].l,5)
		prod = padLeft(convertValue(allRuns[key].p),10)
		mst = padLeft(convertValue(allRuns[key].t),10)
		ms = padLeft(convertValue(allRuns[key].m),10)
...
function statsGrowth() {
	...
	for (let i = 0; i < res.length-1; i++) {
		...
		runs = padLeft(now[1].r,4)
		lvl = padLeft(now[1].l - next[1].l,5)
		prod = now[1].p - next[1].p
		if (prod < 0) {
			prod = next[1].p - now[1].p
			pre = '-'
		}
		prod = padLeft(pre + convertValue(prod),10)
		mst = padLeft(convertValue(now[1].t - next[1].t),10)
		ms = padLeft(convertValue(now[1].m - next[1].m),10)
		...
		data += date + ' | ' +   runs + ' | ' + lvl 
		data += ' | ' + prod + ' | ' + mst + ' | ' + ms
		data += ' | ' + padLeft(avgSp.toFixed(2),5)
		data += ' | ' + padLeft(avgEp.toFixed(2),5) + '\n'
		if ((i+1)%7 === 0) data += sep	
		if (i===20) data += '\n'
	}
	let total = 'Runs: ' + ST.totalRuns.toLocaleString() +  ' | '
...
function getLogData() {
	if (runData) return [runData[2],runData[3],runData[4]]
...
function statsFarm() {
	...
	for (let i = 0; i < data.length; i++) {
		...
		let m = padLeft(data[i][1],9) + ' | '
		let p = padLeft(data[i][2],4) + ' | '
		let s = padLeft(data[i][3],3)
		addStatus2(d + m + p + s)
	...
	addStatus2(seperator)
	avgM = padLeft(convertValue(avgM/data.length),9) + ' | '
	avgP = padLeft(Math.round(avgP/data.length),4) + ' | '
	avgS = padLeft(Math.round(avgS/data.length),3)
...
function getFarmData(all=false) {
	...
	res = Object.keys(res).map(function(k) { return [k, convertValue(res[k][0]),padLeft(res[k][1],4),padLeft(res[k][2],3),padLeft(res[k][3],3)] })
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function missionDaily(pct) {
	if (isNaN(pct) || pct < 1 || pct > 100) return
	info2.value = ''
	let missionPower = calc['mission'] 
	let msDaily1 = msDaily2 = ((calc['msrun'] * maxRuns) + (calc['farmnow'] * 1440))* pct/100
	let msCost1 = calc['msclanCost'] * 100
	let msLevel1 = msLevel2 = calc['msclan'] 
	let rpFarm = 720/500
	let rpDaily1 = rpDaily2 = todayRuns.c ? Math.round(((todayRuns.ep/todayRuns.c) * maxRuns / 500) + rpFarm) : Math.round(50 * maxRuns / 500 + rpFarm)
	...
	for (let i = 1; i <= 22; i++) {			
		...
		let rs = ' - ' + padLeft(rpLevel2,3)
		let ms = ' - ' + padLeft((msLevel2/1000).toFixed(1))
		let c = i === 1 ? '\n' : ''
...
function createMissionUI() {
	missionUI = qs.create(1355,510, 'MS Daily')
	.addNumber('pct', 1, 100, 50, 1, () => missionDaily(missionUI.getValue('pct')))
	.addButton('Calculate ', () => missionDaily(missionUI.getValue('pct')))
	.hideAllTitles()
	.hide()
	.setSize(90,115)
}
...
function msUpgrade() {
	let dollarRun = getLogData()[1], func = arguments.callee.name
	if (typeof logCache[func] !== 'undefined') info2.value = logCache[func]
	$j.get('game/researchanz.php').done(function(r) {
		...
		$j.get('game/megaidleanz.php').done(function(r) {
			...
			addStatus2('Farm: ' + padLeft(farmNow,8) + '  => ' + farmNext) 
			addStatus2(seperator)
			...
			addStatus2('Farm: ' + padLeft(nowFarm,8) + ' => ' + padLeft(newFarm,8)) 
			addStatus2('Run:  ' + padLeft(dollarRun,8) + ' => ' + padLeft(newDollar,8)) 
			addStatus2(seperator)
			addStatus2('RS Upgrade ' + (rsLevel+1))	
			logCache[func] = info2.value 
		})
	})
}
	...
	$j.get('game/megaidleanz.php').done(function(r) {
		...
		let spFarm = Math.ceil(24*6*r.megaidleu3/100)
			$j.get('game/skillanz.php').done(function(r) {	
				...
				addStatus2('                    ' + padLeft(convertValue(sumOld),8))
				addStatus2('MS Farm: ' + padLeft(convertValue(farmCollect),8) + ' / ' + padLeft(convertValue(farm24),8))
				addStatus2('MS Run:  ' + padLeft(convertValue(msRun),8) + ' / ' + padLeft(convertValue(ms24),8))
				...
				addStatus2(seperator)	
				addStatus2('                    ' + padLeft(convertValue(sumNew),8))				
				addStatus2('MS Farm: ' + padLeft(convertValue(farmCollect),8) + ' / ' + padLeft(convertValue(farm24),8))
				addStatus2('MS Run:  ' + padLeft(convertValue(msRun),8) + ' / ' + padLeft(convertValue(ms24),8))		
				addStatus2(seperator)	
		...
		let spChance = Math.ceil(24*6*r.megaidleu3/100)
			$j.get('game/skillanz.php').done(function(r) {	
				...
				for (let i = 1; i <= 22; i++) {
					...
					let msNew = padLeft(convertValue(ms * newSkill),8)
					let pct = padLeft((newSkill*100-100).toFixed(2) + ' %', 7)
					let c = i === 1 ? '\n' : ''
				...
				let pct = padLeft((newSkill*100-100).toFixed(2) + ' %', 7)
				let msNew = padLeft(convertValue(ms * newSkill - ms),8)
...
function prodUpgrade() {
	...
	$j.get('game/researchanz.php').done(function(r) {
		...
		for (let i = 0; i < 22; i++) {
			...
			let c = !i ? '\n' : ''
			addStatus2(sLevel + ': ' + padLeft(nProd, 9) + ' - ' + pct.toFixed(2) + 'x - ' + padLeft(cost,3) + ' rp' + c) 				
		}	
		addStatus2(seperator)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let playerID = { Mahik: 39537838, Bredlox: 44667175, Tudosel: 14718822,	LoveShark: 17596799, 
				Zoomer: 27547311, Gyan: 676336, Alpha: 24838,  pagalla: 39608143 }, playerUI
				 
function showPlayerDay() {
	let acc = playerUI.getValue('playerList').value
	let id = playerID[acc]
	let data = decompress(localStorage.getItem('track_' + acc))
	if (!data) return addStatus2('no player data yet')
	info2.value = ''
	let stats = JSON.parse(data)
	let ct = Object.keys(stats)
	if (ct.length === 1) {
		data = stats[ct[0]]
		addStatus2(seperator)
		addStatus2('MS: ' + data.mega + ' - SP: ' + data.skill)
		addStatus2(ct[0] + ': ' + data.resets)	
		addStatus2(acc)	
	}
	else {
		let msSummary, rsSummary
		for (let i = ct.length-1; i  >= ct.length-2; i--) {
			let current = ct.length-1
			let start = stats[ct[current]]
			let timeStart = convertDate(ct[current])
			let msStart = start.mega, spStart = start.skill
			let timeUpdate = ((convertDate(new Date().toLocaleString()) - timeStart)/1000/3600).toFixed(2) + 'h ago' 
			playerUI.setValue('lastupdate', 'updated: ' + timeUpdate)
			current = i === ct.length-1 ? 0 : ct.length-2
			let end = stats[ct[current]]
			let timeEnd = convertDate(ct[current])
			let msEnd = end.mega, spEnd = end.skill
			let timeTotal = (timeStart-timeEnd)/1000
			let ms = getValue(msStart) - getValue(msEnd)
			let msAvg = ' ('+ convertValue(ms/timeTotal * 3600 * 24) + ')'
			ms = convertValue(ms)
			let sp = spStart - spEnd
			let spAvg = ' ('+ (sp/timeTotal * 3600 * 24).toFixed(2) + ')'
			let rs = start.resets - end.resets
			let rsAvg = ' ('+ Math.floor(rs/timeTotal * 3600 * 24) + ')'
			addStatus2(seperator)
			addStatus2('SP:   ' + sp + spAvg)
			addStatus2('MS:   ' + ms + msAvg)
			addStatus2('RS:   ' + rs + rsAvg)
			addStatus2('Time: ' + displayTime(timeTotal,true))
			msSummary = msStart
			rsSummary = start.resets
		}
		addStatus2(seperator)
		addStatus2(acc + ': ' + msSummary + ' / ' +rsSummary.toLocaleString())		
	}
	document.getElementsByClassName('qs_select')[0].focus()
}

function showPlayerAll() {
	let acc = playerUI.getValue('playerList').value
	let id = playerID[acc]
	let data = decompress(localStorage.getItem('track_' + acc))
	if (!data) return addStatus2('no player data yet')
	...
	let stats = JSON.parse(data)
	let ct = Object.keys(stats)
	let max = ct.length > 16 ? 16 : ct.length
	for (let i = max; i > 0; i--) {
		let date = ct[ct.length-i]
		let ms = padLeft(stats[date].mega, 9)
		let sp = padLeft(stats[date].skill.toLocaleString(), 6) + ' sp'
		date = padLeft(date.split(',')[0], 9).slice(0,4) + ':  '
		addStatus2(date + ms + ' - ' + sp)

	}
	addStatus2(seperator)
	addStatus2(acc + ': ' + stats[ct[ct.length-1]].resets.toLocaleString() + ' resets')	
	document.getElementsByClassName('qs_select')[0].focus()
}
...
function updatePlayer() {
	for (key in playerID) { 
		...
		let id = playerID[key]
		let data = decompress(localStorage.getItem('track_' + acc))
		let canUpdate = true, stats = {}
		if (data) {
			stats = JSON.parse(data)
			canUpdate = (convertDate(new Date().toLocaleString()) - convertDate(Object.keys(stats)[Object.keys(stats).length-1])) /1000/3600 >= 16	
		}
		if (!canUpdate) {
			playerUI.setValue('lastupdate', 'updated: 16 hours')
			break
		}
		else {
			playerUI.disableControl('Update player stats ')
			$j.get('game/playeranz.php?' + id).done(function(r) {
				let rs = r.maxresets
				let ms = r.maxrelikte
				let sp = r.newskillpoints
				let time = new Date().toLocaleString()
				if (data && Object.keys(stats).length > 500) delete stats[Object.keys(stats)[0]] 
				stats[time] = { mega: ms, skill: sp, resets: rs }
				stats = compress(JSON.stringify(stats))
				localStorage.setItem('track_' + acc, stats)	
				playerUI.enableControl('Update player stats ')
				playerUI.setValue('lastupdate', 'updated')
			})	
		}
	}
	document.getElementsByClassName('qs_select')[0].focus()
...
function createPlayerUI() {
	let n = []
	for(key in playerID) n.push(key)
	playerUI = qs.create(1090,420, 'Track player')
	.addDropDown('playerList', n, showPlayerDay)
	.addButton('Show player stats   ', function() {
		if (info2.value.includes('/')) showPlayerAll()
		else showPlayerDay()
	})
	.addButton('Update player stats ', updatePlayer)		
	.addText('lastupdate')
	.disableControl('lastupdate')
	.hideAllTitles()
	.hide()
	.setSize(155,202)	
}
...
function showPlayerUI() {
	playerUI.toggleVisibility()  
	if (!playerUI._hidden) {
		if (!playerUI.getValue('lastupdate')) {
			let acc = Object.keys(playerID)[0]
			let data = JSON.parse(decompress(localStorage.getItem('track_' + acc)))
			let diff = ((convertDate(new Date().toLocaleString()) - convertDate(Object.keys(data)[Object.keys(data).length-1]))/1000/3600).toFixed(2) + 'h ago' 
			playerUI.setValue('lastupdate', 'updated: ' + diff)
		}
		document.getElementsByClassName('qs_select')[0].focus()
	}
}
...
function updateCalc() {
	...
	let pct = '  - ' + padLeft(Math.round(prodNew/prodNow*100),4) + '%'
	info.push(' Prod:  ' + padLeft(convertValue(prodNew),9) + pct)
	...
	msRun = msRun/msNow*msNext
	pct = '  - ' + padLeft(Math.round(msRun/calc['msrun']*100),4) + '%'
	info.push(' Mega:  ' + padLeft(convertValue(msRun),9) + pct)
	...
	pct = '  - ' + padLeft(Math.round((farmDaily/(calc['farmnow'] * farmMax))*100),4) + '%'
	info.push(' Farm:  ' + padLeft(convertValue(farmDaily),9) + pct)
	...
	mission = mission/missionNow*missionNext
	pct = '  - ' + padLeft(Math.round(mission/calc['mission']*100),4) + '%'
	info.push(' Clan:  ' + padLeft(convertValue(mission),9) + pct)
	...
	cc = f > 0 ? cc * f * 100 : 0
	if (tc+cc) info.push(' Cost:  ' + padLeft(convertValue(tc+cc),9))
	tc = 0
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let connTimeOut = 3000, connStatus = 0, connTimer = Date.now()
let errorAudio = new Audio('http://soundbible.com/mp3/analog-watch-alarm_daniel-simion.mp3')
...
function logError(f,r) {
	let e = JSON.parse(localStorage.getItem(account + '_error')) || {}
	e[new Date().getTime()] = {  
	...
	localStorage.setItem(account + '_error', JSON.stringify(e))
}


function padLeft(str, nbr) {
	str = str.toString()
	if (nbr - str.length > 0) str = ' '.repeat(nbr-str.length) + str
	return str
}

function padRight(str, nbr) {
	str = str.toString()
...
function createLog() {
	...
	textf.style.cssText = 'font-family: monospace; font-size: 13px; position: absolute; width: 590px; height: 121px; top: 497px; left: 926px; padding-left: 15px; border-top: none;  border-image: initial; border-radius: 5px; background-color: black; color: rgb(204, 82, 0); caret-color: transparent; overflow: hidden; outline: none; resize: none; cursor: default; border-right: 1px dotted ' + theme + '; border-bottom: 1px dotted ' + theme + '; border-left: 1px dotted ' + theme
...
function createStatus() {
	...
	textf.id = 'info1'
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 290px; height: 380px; top: 44px; left: 1225px; padding-left: 15px; border-top: none; border-right: 1px dotted "+ theme + "; border-bottom: 1px dotted "+ theme + "; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; caret-color: transparent; overflow: hidden; outline: none; resize: none; cursor: default;"
	document.getElementById('maingame').appendChild(textf); 
	textf = document.createElement('div')
	textf.style.cssText = 'position: absolute; left: 1230px; top: 410px; width: 302px; height: 18px; background-color: black;'
	document.getElementById('maingame').appendChild(textf); 	
}
...
function createStatus2() {
    ...
    textf.id = 'info2'
	textf.spellcheck = false
    textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 260px; height: 380px; top: 44px; left: 926px; padding-left: 15px; border-top: none; border-right: 1px dotted "+ theme + "; border-bottom: 1px dotted "+ theme + "; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: transparent; color: "+ theme + "; caret-color: transparent; overflow: hidden; outline: none; resize: none; cursor: default;"
    document.getElementById('maingame').appendChild(textf); 
}
...
function createIdleInfo() {
	for (let i = 1; i <= 6; i++) {
		...
		textf.style.width = '320px'
		textf.style.height = i < 3 ? '122px' : i < 5 ? '184px' : '226px'
		textf.style.top = i < 3 ? '10px' : i < 5 ? '176px' : '402px'
		textf.style.left = i%2 === 1 ? '0px' : '360px'
	    ...
	    document.getElementById('idleinfo' + i).onclick = i > 2 ? getMissionTime : ''
		document.getElementById('idleinfo' + i).oncontextmenu  = function(e) { return false }
	}
	idleinfo2.onclick = () => idlemode ? idlemodeend() : startIdleMode()
}
...
function createTimeInfo() {
	...
	textf.id = id
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 14px; line-height: 18px; position: absolute; width: 146px; padding-left: 20px; height: 76px; top: 106px; left: 10px; border: 1px dotted "+ theme + "; border-radius: 5px; background-color: black; color: "+ theme + "; caret-color: transparent; outline: none; overflow: hidden; resize: none; cursor: default;"
	document.getElementById('maingame').appendChild(textf); 
}
...
function createHeader() {
	...
	textf.disabled = true
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 260px; height: 34px; top: 10px; left: 926px; padding-left: 15px; padding-top: 3px; border-top: 1px dotted "+ theme + "; border-right: 1px dotted "+ theme + "; border-bottom: none; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; overflow: hidden; resize: none; cursor: default; outline: none; caret-color: transparent"
	document.getElementById('maingame').appendChild(textf); 
	header1.value = 'STATS'
	...
	textf.disabled = true
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 290px; height: 34px; top: 10px; left: 1225px; padding-left: 15px; padding-top: 3px; border-top: 1px dotted "+ theme + "; border-right: 1px dotted "+ theme + "; border-bottom: none; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; overflow: hidden; resize: none; cursor: default; outline: none; caret-color: transparent"
	document.getElementById('maingame').appendChild(textf); 
	header2.value = 'LOG'
	textf = document.createElement('textarea');
	textf.id = 'header3'
	textf.disabled = true
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 590px; height: 34px; top: 463px; left: 926px; padding-left: 15px; padding-top: 3px; border-top: 1px dotted "+ theme + "; border-right: 1px dotted "+ theme + "; border-bottom: none; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; overflow: hidden; resize: none; cursor: default; outline: none; caret-color: transparent"
	document.getElementById('maingame').appendChild(textf); 
...
function createLines() {
	...
	l(1,12,34,300,'idlemode')
	l(2,374,34,300,'idlemode')
	l(3,12,200,300,'idlemode')
	l(4,374,202,300,'idlemode')
	l(5,12,425,300,'idlemode')
	l(6,374,425,300,'idlemode')	
	l(7,941,32,250,'maingame')
	l(8,1240,32,282,'maingame')
...
function logBackup() {
	...
	data = ''
	for (key in playerID) { 
		let current = decompress(localStorage.getItem('track_' + key))
		data += 'var data = ' + current + '\n'
		data += 'data = compress(JSON.stringify(data))\n'
		data += "localStorage.setItem('track_" + key + "',data)\n\n"
	}
	file = new Blob([data], {type: 'text/plain'})
	a.href = URL.createObjectURL(file)
	a.download = account + '_track.txt'
	a.click()
	data = 'var data = ' + decompress(localStorage.getItem(account + '_summary')) + '\n'
	data += 'data = compress(JSON.stringify(data))\n'
	data += 'localStorage.setItem(\'' + account +	'_summary\', data)'
	file = new Blob([data], {type: 'text/plain'})
	a.href = URL.createObjectURL(file)
	a.download = account + '_summary.txt'
	a.click()
	data = 'var set = ' + JSON.stringify(ST) + '\n'
	data += 'localStorage.setItem(\'' + account +	'_settings\', JSON.stringify(set))'
	file = new Blob([data], {type: 'text/plain'})
	a.href = URL.createObjectURL(file)
...
function createInfos() {
	...
	log.onkeydown = function(e) { 
			if (e.keyCode === 38) { log.scrollTop -= 120; event.preventDefault() }
			else if (e.keyCode === 40) { log.scrollTop += 120; event.preventDefault() }
		}
	...
	info2.onmousedown  = function(e) {
		if (e.which === 3) { 
			let c = runData.length ? true : false
			statusInfo(c)
		}
	}
...
function loadLog() {
	let data = decompress(localStorage.getItem(account + '_log'))
...
function saveLog() {
	let data = log.value
	if (!data) return
	localStorage.setItem(account + '_log', compress(data))
}
...
function loadStatus() {
	let data = decompress(localStorage.getItem(account + '_status'))
...
function saveStatus() {
	...
	if (!data) return
	data = data.split('\n')
	for (let i = data.length-1; i >= 5000; i--) { data.splice(i,1) }	
	data = data.join('\n')
	localStorage.setItem(account + '_status', compress(data))
	info1.value = data
}
...
function saveSettings(quick=false) {
	...
	localStorage.setItem(account +	'_settings',  JSON.stringify(ST))	
}
	
function loadSettings() { 
	ST = JSON.parse(localStorage.getItem(account + '_settings'))
	if (ST) {
		buyMax = ST.buyMax
		buyGoal = ST.buyGoal
...
function createButtons() {
	...
	'Settings','Backup', 'Theme','Track','Mission','Farm', 'Idle']
	...
	btn1.onmousedown  = function(e) {
		if (e.which === 1) startAutoPlay()
		else if (e.which === 3) startAutoPlay(false)	
	}
	btn2.onclick = function() {
		if (!finishNext) finishNext = true
	...
	btn4.onclick  = function() {
		if (!sleepOn) {
			sleepOn = true
			btn4.innerText = timeFormat(sleepTime[0]) + ':' + timeFormat(sleepTime[1]) 		
			}
		else {
			sleepOn = false
			btn4.innerText = 'Sleep'
		}
		highlightBtn(btn4)		
	}
	btn5.onclick = function() {
		if (!ST.audio) ST.audio = true
		else ST.audio = false
		highlightBtn(btn5)
		saveSettings(true)
	}
	...
	btn13.onclick  = function(e) {
		missionUI.toggleVisibility()
		if (!missionUI._hidden) missionDaily(missionUI.getValue('pct'))	
		highlightBtn(btn13)
	}
	...
	btn19.onclick = function() {
		missionLeft = 0
		doMission(true)
		addStatus2('Checking mission')	
		highlightBtn(btn19,true) 
	}
	btn20.onmousedown = function(e) {
		if (e.which === 1) statsFarm()
		else if (e.which === 3) collectNow()		
	}
...
document.onkeydown = function(e) {
	if (!isVisible('guild')) {
		...
		if (!id.includes('log') && !id.includes('info') && !id.includes('stats') && UI[0]._hidden && costUI[0]._hidden) {
			...
			else if (e.keyCode === 84) showPlayerUI()  //t
...
function setTheme() {
	...
	div.style.cssText = 'height: 89px; background-color: transparent; border: 1px dotted ' + theme 
	allDivs.push(div)
	div = document.getElementsByClassName('epbartxt')[0]
	div.style.cssText = 'color: ' +theme + '; font-weight: 200;'
	allDivs.push(div)	
	...
	div.style.cssText = 'color: ' +theme + '; font-weight: 200; font-family: monospace; font-size: 14px; margin-top: 6px;'
	allDivs.push(div)
    document.getElementsByClassName('guildname')[0].remove()
    div = document.getElementsByClassName('nextrstimeranz')[0]
	div.style.cssText = 'color: ' +theme + '; font-weight: 200; font-family: monospace; font-size: 11px;'
	allDivs.push(div)
	div = document.getElementById('goldbar')
	div.style.cssText = 'width: 322px; height: 26px; background-color: transparent; border: 1px dotted ' + theme 
	allDivs.push(div)
	...
	div.style.cssText = 'width: 326px; height: 26px; left: -16px; background-color: transparent; border: 1px dotted ' + theme 
	allDivs.push(div)	
	div = document.getElementsByClassName('gems')[0]
	div.style.cssText = 'width: 326px; line-height: 26px; text-align: left; font-size: 16px; font-family: monospace; font-weight: 200; margin-left: 16px; color: ' + theme
	allDivs.push(div)	
	...
	for (var i = 0; i < div.length; i++) { 
		div[i].style.cssText = 'color: ' + theme + ';background: transparent; text-align: left; font-weight: 200; font-family: monospace; font-size: 14px; text-shadow: none; cursor: default; border: 1px dotted ' + theme 
	...
	div.style.cssText = 'color: ' + theme + '; background: transparent; text-align: left; font-weight: 200; font-family: monospace; font-size: 14px; text-shadow: none; width: 168px;  border: 1px dotted ' + theme 
	...
	allDivs.push(log,info1,info2,idleinfo1,idleinfo2,idleinfo3,idleinfo4,idleinfo5,idleinfo6,statswin,timeinfo,header1,header2,header3)	
	allDivs.push(idlelin1,idlelin2,idlelin3,idlelin4,idlelin5,idlelin6,idlelin7,idlelin8,idlelin9)
	buildingcostanz(true)
	if (ST.audio) highlightBtn(btn5)
	if (ST.idle) highlightBtn(btn21)
}

function changeTheme() {
	if (theme === 'rgb(204, 82, 0)') theme = 'rgb(240, 132, 0)'
	else if (theme === 'rgb(240, 132, 0)') theme = 'rgb(0, 153, 0)'
	else if (theme === 'rgb(0, 153, 0)') theme = 'rgb(0, 122, 204)'
...
let lastBuy = Array(8).fill(0)
function buildingbuy(nbr, amt=0) {
	  if (!amt) amt = aktbuystufe
	  let canBuy = getGold()-getValue(bcost[(nbr*4)-(4-amt)])	
	  if (Date.now() - buyStepTimer > 10000 && autoPlay) startIdleMode()
	  else if (canBuy > 0 && !lastBuy[nbr-1]) {
			lastBuy[nbr-1] = true
			buyStepTimer = Date.now()
			document.getElementsByClassName('gold')[0].innerText = convertValue(canBuy)
			$j.get('game/buildingbuy.php?b=' + nbr + '&s=' + amt, function(e) {
				if (e.erfolg == 1) {
					building = new Array()
					bcost = new Array()
					maxbplus = new Array()
...
function researchanz() {
	...
	$j.get('game/researchanz.php', function(e) {
		if (e.erfolg == 1) {
			...
			$j('.researchpoints').html('Researchpoints: ' + e.researchpoints + '<br>(use Researchpoints to skip a research)');
			$j('.researchbonis').html('<div class="rsplatz1"></div><span class="yourstats">Research Bonus:</span><div class="rsplatz1"></div>' +
								nextbonustxt);
			$j('.researchbtn').html('Research: ' + (e.researchpoints || 0))
...
function researchup(e) {
	confirmBox(function(){
		0 == rsupsperre && (rsupsperre = 1, $j.get("game/researchup.php?" + e, function(a) {
        1 == a.erfolg ? researchanz() : 2 == a.erfolg && ($j(".researchfail" + e).html("Not enough Researchpoints!"), rsupsperre = 0)
		}))	 
	})
}
...
function skillanz() {
    resetspstatus = 0, chatoffen = 0, $j.get("game/skillanz.php", function(e) {
        if (1 == e.erfolg) {
            for (skillplus = 0, $j(".gold").html(e.gold.slice(2,12)), e.skillpoints > 0 ? $j(".skillbtn").html("Skills: " + e.skillpoints) : $j(".skillbtn").html("Skills"), $j(".sptxt").html(e.skillpoints), $j(".resetsp").html("(reset for 10 Goldbars)"), spbonus = new Array, spcost = new Array, skill = new Array, skillnext = new Array, skill = e.skill, spbonus = e.spbonus, spcost = e.spcost, skillnext = e.skillnext, i = 1; i < 11; i++) i < 9 ? ($j(".skilllvl" + i).html("Lv." + skill[i] + " (next: x" + skillnext[i] + " Bonus)"), $j(".skilltxt" + i).html("Bonus:<br>x" + spbonus[i] + spbonustxt[i] + "<br>Upgrade for " + spcost[i])) : 9 == i ? ($j(".skilllvl" + i).html("Lv." + skill[i] + " (next: x" + skillnext[i] + " Bonus)"), $j(".skilltxt" + i).html("Bonus: x" + spbonus[i] + spbonustxt[i] + "<br>Upgrade for " + spcost[i])) : 10 == i && (skillnext2 = spbonus[i] + 10, $j(".skilllvl" + i).html("Lv." + skill[i] + " (next: +" + skillnext2 + "%)"), $j(".skilltxt" + i).html("Bonus: gain " + spbonus[i] + spbonustxt[i] + "<br>Upgrade for " + spcost[i]));
            closeall(), $j("#upgrades").css({
                display: "block"
            })
			for (let i = 2; i <= 7; i++) {
...
function eventanz() {
  (chatoffen = 0),
    $j.get("game/eventanz.php", function(e) {
      1 == e.erfolg &&
        ($j("#events").html(
          '<div class="eventtxt"><br>' +
            e.eventtxt +
            '</div><div class="storetitle">Event-Store:</div><div class="esplatz"></div>' +
            e.eventstorevar
        ),
        closeall(),
        0 == e.noreward
          ? $j(".drclaim" + e.nextreward).html(
              '<div class="drclaimbtn" onclick="claimreward()">claim</div>'
            )
          : $j(".drclaim" + e.nextreward).html(
              '<div class="drclaimbtn2">claim</div>'
            ),
        e.eventpoints > 0
          ? 0 == e.noreward
            ? $j(".eventbtn").html("Event: " + e.eventpoints)
            : $j(".eventbtn").html("Event: " + e.eventpoints)
          : 0 == e.noreward
            ? $j(".eventbtn").html("Event")
            : $j(".eventbtn").html("Event"),
        1 == eventgot &&
          ((eventgot = 0),
          $j("#maintutorial").css({
          ...
          $j(".tutorialtxt").html(eventgottxt)),
        $j("#events").css({
          display: "block"
        }));
    });
...
function eventbuy(e) {
	confirmBox(function(){
		$j.get("game/eventbuy.php?" + e, function(a) {
			1 == a.erfolg && (a.skillpoints > 0 ? $j(".skillbtn").html("Skills: " + a.skillpoints) : $j(".skillbtn").html("Skills"), a.researchpoints > 0 ? ($j(".researchbtn").html("Research: " + a.researchpoints), a.researchfinish > 0 && $j(".researchbtn").html("Research")) : a.researchfinish > 0 ? $j(".researchbtn").html("Research") : $j(".researchbtn").html("Research"), a.eventpoints > 0 ? 0 == a.noreward ? $j(".eventbtn").html("Event: " + a.eventpoints) : $j(".eventbtn").html("Event: " + a.eventpoints) : 0 == a.noreward ? $j(".eventbtn").html("Event") : $j(".eventbtn").html("Event"), 4 == e && (eventgot = 1, eventgottxt = a.pricetxt), $j(".gold").html(a.gold.slice(2,12)), $j(".gems").html(a.gems), eventanz())
		})
	})
}
...
function clanquests() {
    $j.get("game/clanquests.php", function(e) {
        if (1 == e.erfolg) {
            for (cqfrage = 0, cqsperre = 0, chatoffen = 0, ownrank = e.clanrang, clearTimeout(timergc), guildrankvar = "", namearray = new Array, maxlevelarr = new Array, aktclandmg = new Array, lastclandmg = new Array, lastclandmg = e.lastclandmg, aktclandmg = e.aktclandmg, namearray = e.namearray, maxlevelarr = e.maxlevelarr, memberanzvar = e.anz, realnextcost = e.uaktanz, i = 1; i <= memberanzvar; i++) guildrankvar += '<div class="cqname">' + namearray[i] + '</div><div class="cqlvl">' + maxlevelarr[i] + '</div><div class="cqcurrent">' + aktclandmg[i].toLocaleString() + '</div><div class="cqlast">' + lastclandmg[i].toLocaleString() + "</div>";
            $j(".gems").html(e.gems), nextmissionnmbr = e.clanquests + 1, $j("#guild").html('<div class="clananzname">' + e.clanname + " &nbsp;&nbsp; Clangold: " + e.clangold + " &nbsp;&nbsp; Clan-Missions: " + e.clanquests + '</div><div class="clananzall"><div class="platz1px"></div><div class="clbtn1" onclick="guildanz()">Overview</div><div class="clbtn2" onclick="chatanz()">Chat</div><div class="clbtn3" onclick="memberanz()">Member</div><div class="clbtn4" onclick="clanquests()">Clan-Missions</div><div class="clbtn5" onclick="clanranking()">Highscore</div><div class="clanmissions">Help your Clan to finish Mission ' + nextmissionnmbr + ":<br>" + e.aktclanquest + " / " + e.aktfullpoints + " Points<br>Add " + e.clanpoints + ' to this Mission:<div class="addcqbtn" onclick="cqattack2()">participate' + e.nextcost + '</div></div><div class="cqranking"><div class="clanchat2"><div class="cqname"><u>Name</u></div><div class="cqlvl"><u>max. Level:</u></div><div class="cqcurrent"><u>Current Points:</u></div><div class="cqlast"><u>Last Mission Points:</u></div>' + guildrankvar + "</div></div></div>"), e.newclanchat > 0 || e.clannews > 0 || e.clannews2 > 0 ? $j(".clanbtn").html("Fastfood Clan") : $j(".clanbtn").html("Fastfood Clan"), clanquestdauer = e.clanquestdauer, clanquestdauer > 0 && (clearTimeout(cqtimervar), cqtimer())
			var div = $j('.cqname')
			for (var i = 0; i < div.length; i++) {
  ...
  (chatoffen = 0),
    $j.get("game/storeanz.php", function(e) {
      1 == e.erfolg &&
        ...
        e.skillpoints > 0
          ? $j(".skillbtn").html("Skills: " + e.skillpoints)
          : $j(".skillbtn").html("Skills"),
        e.researchpoints > 0
          ? $j(".researchbtn").html("Research: " + e.researchpoints)
          : $j(".researchbtn").html("Research"),
        closeall(),
        $j("#store").css({
          display: "block"
        }));
