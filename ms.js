...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let sVersion = 'v.1.072', sLoaded = false
let maxRuns = 80
...
function resetRoutine() {
	...
	$j.get('game/megaanz.php').done(function(r) {
		...
		incomeoffline.slice(2,12), r.nextreset, r.maxrelikte], new Date().toLocaleTimeString().slice(0,5))		
...
function adjustProd() {
	...
	if ((Date.now()- autoPlay)/1000 > sessionTime) {
		if (runData[runData.length-1][0] < maxLevel && prodExeeded) factor = 1.01
	 	else if (runData[runData.length-1][0] > maxLevel || (runData[runData.length-1][0] === maxLevel && runData[runData.length-1][1] > 80)) factor = 0.99
		if (factor) {
	...
	if (runData[runData.length-1][0] > highLevel) { 
		highLevel = runData[runData.length-1][0]
		saveSettings()
	}
}
...
function displayStats() {
	...
	if (isVisible('idlemode') || isVisible('afkwin')) {
	    ...
	    if (isVisible('idlemode')) $j('#ii2').val(i2.join('\n'))
	 	else {
			 if (runData.length) {
				let max = 0
				 for (let i = runData.length-1; i >= 0; i--) {
					...
					i2.push(cur[0] + ' | ' + todayRuns.r + ' | ' + cur[2] + ' | ' +  cur[3] + ' | ' +  cur[4] + '\n')
				 }
