...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let sVersion = 'v.0.847', sLoaded = false
let maxRuns = 125, mainBuild = 8
...
function displayStats() {
	...
	if (isVisible('idlemode') || isVisible('afkwin')) {
	    ...
	    if (!isVisible('afkwin')) $j('#ii2').val(i2.join('\n'))
			 ...
			 if (runData.length) {
				 for (let i = runData.length-1; i >= runData.length-2; i--) {
					let cur = runData[i]
...
function getMissionStanding(r) {
	let info = [] 
	let pct = r.aktclanquest ? pl(Math.floor(getValue(r.aktclanquest)/getValue(r.aktfullpoints)*100) + '%', 5) + pl(r.aktfullpoints, 29) : ''
...
function statsSummary() {
	...
	data = '  TIME   | RUN  | LEVEL |    PROD    |  MS TOTAL  |     MS     |   SP  |   EP \n' + sep + data
...
function createAFKwin() {
	...
	textf.id = 'afkwin'
	textf.spellcheck = false
	textf.style.cssText = 'font-family: monospace; font-size: 50px; position: absolute; width: 1312px; height: 720px; top: 0px; left: 0x; padding-left: 220px; border-top: none;  border-image: initial; border-radius: 5px; background-color: black; color: ' + theme + '; caret-color: transparent; overflow: hidden; outline: none; resize: none; cursor: default; border: 1px dotted black; display: none;'
	document.getElementById('maingame').appendChild(textf); 
...
function createButtons() {
	...
	'Cost UI','MS Upgrade','MS Growth','Daily','Production','Clan stats','Research',
	'Statistics','Calculator', 'Set theme', sVersion ,'Farm stats','Collect', 'Backup']
	...
	btn13.onmousedown = function(e) {
		...
		else if (e.which === 3) showClanRanking()
	}
