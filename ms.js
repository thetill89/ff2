...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
let sVersion = 'v.1.06', sLoaded = false
let maxRuns = 80

initScript()
function initScript() {
	//if (document.getElementsByClassName('eventbtn').length && typeof hundredbplus === 'object' ) {
...
function adjustProd() {
	...
	if ((Date.now()- autoPlay)/1000 > sessionTime) {
		if (runData[0] < maxLevel && prodExeeded) factor = 1.01
...
function finishRun() {
	...
	setTodayRuns()
	updateSummary()	
	addLog(pl(todayRuns.r, 3) + ' | ' + pl(runData[0],5) + ' | ' + pl(runData[2],9) + ' | '
	+ pl(runData[3],9) + ' | '  + pl(runData[4],10) + ' | '
...
function statusInfo() {
	let s = 11
	info2.value = ''
	if (runData.length) addStatus2(pr('Last:',s) + runData[0] + ', ' + Math.floor(runData[1]) + '%')
...
function getMissionStanding(r) {
	let info = [] 
	let pct = r.aktclanquest ? pl(Math.floor(getValue(r.aktclanquest)/getValue(r.aktfullpoints)*100) + '%',4) + ' '.repeat(23) + r.aktfullpoints : pl('0%',4)
	...
	if(autoReward && (!new Date().getHours() || forced)) {
		$j.get('game/eventanz.php').done(function(r) { 
			if (!r.noreward) {
				...
				$j.get('game/claimreward.php').done(function(r) { homeanz() })	
			}
		})	
	}
}
...
function statsGrowth() {
	...
	for (let i = 0; i < res.length-1; i++) {
		...
		ms = pl(convertValue(now[1].m - next[1].m),10)
...
function showPlayerDay() {
	...
	let data = decompress(load('track_' + acc))
...
function showPlayerAll() {
	...
	let stats = JSON.parse(data)
	let ct = Object.keys(stats)
	let max = ct.length > 22 ? 22 : ct.length
	for (let i = max; i > 0; i--) {
		let date = ct[ct.length-i]
		let ms = pl(stats[date].mega, 9)
		let sp = pl(stats[date].skill.toLocaleString(), 6) + ' sp'
		date = pl(date.split(',')[0], 9).slice(0,4) + ':  '
	...
	for (key in playerID) { 
		...
		if (!canUpdate) {
			...
			$j.get('game/playeranz.php?' + id).done(function(r) {
				...
				if (Object.keys(stats).length > 1000) delete stats[Object.keys(stats)[0]] 
				stats[time] = { mega: ms, skill: sp, resets: rs }
				save('track_' + acc, compress(JSON.stringify(stats)))	
...
function showPlayerUI() {
	...
	if (!playerUI._hidden) {
		if (!playerUI.getValue('lastupdate')) {
			...
			let data = JSON.parse(decompress(load('track_' + acc)))
...
function createAFKwin() {
	...
	textf.id = 'afkwin'
	textf.spellcheck = false
	textf.style.cssText = 'font-family: monospace; font-size: 50px; position: absolute; width: 1382px; height: 720px; top: 0px; left: 0x; padding-left: 150px; border-top: none;  border-image: initial; border-radius: 5px; background-color: black; color: ' + theme + '; caret-color: transparent; overflow: hidden; outline: none; resize: none; cursor: default; border: 1px dotted black; display: none;'
	document.getElementById('maingame').appendChild(textf); 
...
function createHeader() {
	...
	textf.disabled = true
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 258px; height: 34px; top: 10px; left: 926px; padding-left: 15px; padding-top: 3px; border-top: 1px dotted "+ theme + "; border-right: 1px dotted "+ theme + "; border-bottom: none; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; overflow: hidden; resize: none; cursor: default; outline: none; caret-color: transparent"
	document.getElementById('maingame').appendChild(textf); 
	header1.value = 'STATS'
	...
	textf.disabled = true
	textf.spellcheck = false
	textf.style.cssText = "font-family: monospace; font-size: 13px; position: absolute; width: 285px; height: 34px; top: 10px; left: 1230px; padding-left: 15px; padding-top: 3px; border-top: 1px dotted "+ theme + "; border-right: 1px dotted "+ theme + "; border-bottom: none; border-left: 1px dotted "+ theme + "; border-image: initial; border-radius: 5px; background-color: black; color: "+ theme + "; overflow: hidden; resize: none; cursor: default; outline: none; caret-color: transparent"
	document.getElementById('maingame').appendChild(textf); 
	header2.value = 'LOG'
	textf = document.createElement('textarea');
	textf.id = 'header3'
...
function logBackup() {
	...
	data = ''
	for (key in playerID) { 
		let current = decompress(load('track_' + key))
		data += 'var data = ' + current + '\n'
		data += 'data = compress(JSON.stringify(data))\n'
		data += "save('track_" + key + "',data)\n\n"
	}
...
function setTheme() {
	...
	allDivs.push(log,info1,info2,statswin,timeinfo)
